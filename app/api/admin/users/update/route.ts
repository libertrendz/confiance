...
export async function POST(req: Request) {
  try {
    ...
    if (callerRole !== 'admin' && callerRole !== 'gestor') return NextResponse.json({ error: 'forbidden' }, { status: 403 });
    ...
    // sync metadata (opcional, mas ajuda no menu)
    const um = await admin.auth.admin.updateUserById(userId, { user_metadata: { app_role: papel } });
    if (um.error) return NextResponse.json({ error: 'metadata_failed', details: um.error.message }, { status: 400 });
