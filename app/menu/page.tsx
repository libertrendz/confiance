...
export default function MenuPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setRole((meta.app_role as AppRole) || 'colaborador');
      setReady(true);
    })();
    return () => { alive = false; };
  }, [supa]);
    ...
    <main style={{ padding: 12, fontFamily: 'system-ui', maxWidth: 1100, margin: '0 auto' }}>
      <header style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ lineHeight: 1.1 }}>
          <div style={{ fontWeight: 700, fontSize: 16, color: '#0A3D91' }}>{shortName(email)}</div>
        ...
        <div style={{ textAlign: 'right' }}>
          <button onClick={sair} style={{ padding: '8px 12px', borderRadius: 10, border: '1px solid #ddd', background: '#fff', cursor: 'pointer', whiteSpace: 'nowrap' }} aria-label="Terminar sessão">Sair</button>
        </div>
      </header>
