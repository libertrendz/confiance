...
function normalizeSalarioTipo(value: string | null): SalarioTipo {
  if (value === 'hora' || value === 'dia' || value === 'mensal') {
    return value;
  }
...
export default function ColaboradorEditPage({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!j.ok) {
          throw new Error(j.error || 'Falha ao carregar colaborador');
        }
        ...
        if (alive) {
          ...
          setRecord({
            ...r,
            salario_tipo: normalizeSalarioTipo(r.salario_tipo),
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      const salarioTipo = normalizeSalarioTipo(record.salario_tipo);
      ...
      const payload = {
        ...
        custo_hora: salarioTipo === 'hora' ? record.custo_hora : null,
        custo_dia: salarioTipo === 'dia' ? record.custo_dia : null,
        salario_atual:
          salarioTipo === 'mensal' ? record.salario_atual : null,
      ...
      if (!j.ok) {
        throw new Error(j.error || 'Falha ao guardar colaborador');
      }
